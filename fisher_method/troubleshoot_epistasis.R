...
DF_ex <- read.delim("Example_genotype_data.tsv", header=TRUE, stringsAsFactors=FALSE)


DF_test <- read.delim("dauer_gene2.tsv", header=TRUE, stringsAsFactors=FALSE)


all.data_t = data.frame(matrix(vector(), 0, 2,
                             dimnames=list(c(), c("X1", "X2"))),
                      stringsAsFactors=F)

all.genes_t <-list() #loop will fill all.genes with all genes in dataset

for (h in names(DF_ex)){
  list<-na.omit(DF_ex[, h], stringsAsFactors=FALSE) 
  all.genes_t<-append(all.genes_t, list)
} 

df<-data.frame(matrix(unlist(all.genes_t),nrow=length(all.genes_t), byrow=T), stringsAsFactors=FALSE)
...
for(i in names(DF_test)){
  ...
  all.data<-rbind(all.data_t, pairs.to.df, stringsAsFactors=FALSE)
}


list <- na.omit(DF_test[, "BRC20067"])
generatepairs_check <- combn(list, 2, simplify=FALSE, stringsAsFactors=FALSE)
